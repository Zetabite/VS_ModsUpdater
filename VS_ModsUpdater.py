...
lang_handler = languagehandler.LanguageHandler("en_US")
logger = logging.getLogger('VS_ModsUpdater')


def log_info(rprint_msg: str="%s", logger_msg: str="%s", args: list = [""]):
    args = [str(a) for a in args]
    rprint(rprint_msg % tuple(args))
    logger.info(logger_msg, *list(args))


def log_warning(rprint_msg: str="%s", logger_msg: str="%s", args: list = [""]):
    args = [str(a) for a in args]
    rprint(rprint_msg % tuple(args))
    logger.warning(logger_msg, *list(args))


def log_error(rprint_msg: str="%s", logger_msg: str="%s", args: list = [""]):
    args = [str(a) for a in args]
    rprint(("[red]%s[/red]\n" % lang_handler.get("error_msg")) + (rprint_msg % tuple(args)))
    logger.error(logger_msg, *list(args), exc_info=1)


def log_debug(rprint_msg: str="%s", logger_msg: str="%s", args: list = [""]):
    args = [str(a) for a in args]
    rprint(rprint_msg % tuple(args))
    logger.debug(logger_msg, *list(args))
...
class VSUpdate:
    def __init__(self):
        ...
        if pathhandler.get_configfile_path() is None or not pathhandler.get_configfile_path().is_file():
            if args.nopause == "false":
                log_info("\n\t\t[bold cyan]%s[/bold cyan]\n", "%s", [lang_handler.get("first_launch_title")])
                i = 1
            ...
            log_info("\n\t[bold cyan]%s[/bold cyan]:", "%s", [lang_handler.get("first_launch_config_done")])
            log_info("\t\t- [bold cyan]%s[/bold cyan]: %s", "%s: %s", [lang_handler.get("first_launch_lang_txt"), self.lang_name])
            log_info("\t\t- [bold cyan]%s: %s[/bold cyan]", "%s: %s", [lang_handler.get("first_launch_pathmods"), pathhandler.get_mods_path()])
            log_info("\t\t- [bold cyan]%s[/bold cyan]:", "%s", [lang_handler.get("first_launch_game_ver_max")])
            log_info("\t\t- [bold cyan]%s: %s[/bold cyan]:", "%s: %s", ["force_update", self.force_update])
            log_info("\t\t- [bold cyan]%s: %s[/bold cyan]:", "%s: %s", ["disable_mod_dev", self.disable_mod_dev])

            # On demande de continuer ou on quitte
            if args.nopause == "false":
                log_info("\n\t[bold cyan]%s[/bold cyan]", "%s", [lang_handler.get("first_launch2")])
    ...
    def get_changelog(url):
        ...
        try:
            ...
            log_error("%s", "%s", ["ReadTimeout error: Server did not respond within the specified timeout."])
    ...
    def update_mods(self):
        ...
        for mod_maj in self.liste_mod_maj_filename:
            ...
            try:
                ...
                log_info(
                    "[green]%s[/green]: %s : %s - %s : %s", "%s: %s : %s - %s : %s",
                    [modname_value, lang_handler.get("compver1"), self.version_locale, lang_handler.get("compver2"), self.mod_last_version_online]
                )
                ...
                if self.disable_mod_dev == "false" or mod_prerelease_value.prerelease is None:
                    ...
                    if result_game_compare_version == -1 or result_game_compare_version == 0:  # On met à jour
                        if result_compversion_local == -1 or (result_compversion_local == 0 and self.force_update.lower() == "true"):
                            ...
                            log_info(
                                "\t%s: %s %s", "\t%s: %s %s",
                                [lang_handler.get("compver3"), file_size_mo, lang_handler.get("compver3a")]
                            )
                            log_info(
                                "\t[green] %s v.%s[/green] %s", "\t %s v.%s %s",
                                [modname_value, self.mod_last_version_online, lang_handler.get("compver4")]
                            )
                ...
                # Affiche de l"erreur si le lien n"est pas valide
                log_error("%s: %s (%s)", "%s: %s (%s)", [err_url.reason, modname_value, mod_url_api])
            except KeyError:
                log_error("%s\n%s", "%s\n%s", [modname_value, "Mod might be local or has been deleted online"])
            except Exception:
                log_error("%s\n%s", "%s\n%s", [modname_value, "Mod might be local or has been deleted online"])
    ...
    def resume(self):
        ...
        if len(self.mods_exclu) > 1:
            ...
            for k in range(0, len(self.mods_exclu)):
                ...
                log_info(" - [red]%s v.%s[/red]", " - %s v.%s", [modinfo_values[0], modinfo_values[2]])
...
def main(args):
    logpath: str = f"VS_ModsUpdater-{datetime.today().strftime('%Y-%m-%d-%H-%M-%S')}.log"
    logpath: Path = Path(pathhandler.get_logs_path(), logpath)
    ...
    if args.nopause == "false" or args.makepdf == "true":
        ...
        if make_pdf == lang_handler.yesno(0) or make_pdf == lang_handler.yesno(2):
            ...
            log_info(
                "\t[green]%s[/green]\n\t[green]* %s *[/green]\n\t[green]%s[/green]\n",
                "\t%s\n\t* %s *\n\t%s", [string_asterisk, lang_handler.get("makePDFTitle"), string_asterisk]
            )
            ...
            for modfilepath in glob.glob(str(mod_files_path)):
                if os.path.splitext(modfilepath)[1] == ".zip" or os.path.splitext(modfilepath)[1] == ".cs":
                    ...
                    log_info("\t\t%s %s/%s", "\t\t%s %s/%s", [lang_handler.get("addingmodsinprogress"), nb_mods_ok, nb_mods])
            pdf = MakePdf()
            pdf.makepdf()
